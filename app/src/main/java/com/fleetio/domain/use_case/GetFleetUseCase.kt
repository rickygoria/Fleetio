...
class GetFleetUseCase @Inject constructor(
    private val repo: VehicleFleetApi
) {
    private val TAG = "GetFleetUseCase"

    operator fun invoke(): Flow<ApiResponse<List<VehicleDetail>>> = flow {
        try {
            emit(ApiResponse.Loading<List<VehicleDetail>>(null))
            val fleet = repo.getFleet()
